...
int main(int argc, char** argv) {
	...
	char string_res[1024];
	memset(string_res,'\0',1024);
	int counter=0;
	sprintf(&string_res[counter++],"%d",3);
	sprintf(&string_res[counter++],".");

	int DIGITS = 250;

	for (int i=0;i<DIGITS;i++) {
		int res = notmain(i+1);
		int first = firstDigit(res);
		sprintf(&string_res[counter++],"%d",first);
	}
	printf("%d digits of PI %s\n",DIGITS, string_res);
	return 0;
}
